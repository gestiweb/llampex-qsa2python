...
function interna_calculateField(fN):String
{
		...
		switch(fN) {
				...
				\end */
						res = util.sqlSelect("co_subcuentas", "coddivisa",
								"idsubcuenta = " + cursor.valueBuffer("idsubcuenta"));
						if (!res)
				...
				*/
				...
				\end */
						if (this.child("fdbCodSubcuenta").value().length == this.iface.longSubcuenta) {
								...
								var idCuenta:Number = util.sqlSelect("co_subcuentas", "idcuenta", 
									"codsubcuenta = '" + this.child("fdbCodSubcuenta").value() + 
									"' AND codejercicio = '" + this.iface.ejercicioActual + "'");
								if (!idCuenta)
									return;
								...
								this.iface.idCuentaEsp = util.sqlSelect("co_cuentas", "idcuentaesp", 
										"idcuenta= " + idCuenta);
