		...
		for ( contador = 0; contador < totalPrepartidas; contador++) {
			...
			if ( tipoImporte == "Calcular" ) {
				...
				if (curPlanPartidas.valueBuffer("debeohaber") == "Debe") {
					if ( contador == totalPrepartidas-1 ) {
						...
						curPartidas.setModeAccess(curPartidas.Edit);
						curPartidas.refreshBuffer();
						curPartidas.setValueBuffer("debe", parseFloat(valorFinal));
						if (this.iface.esDivisaExt == true) {
							if (parseFloat(curPartidas.valueBuffer("tasaconv")) != 0) {
								this.iface.bloqueo = true;
								curPartidas.setValueBuffer("debeme",(parseFloat(valorFinal) / parseFloat(curPartidas.valueBuffer("tasaconv"))));
								this.iface.bloqueo = false;
							}
						}
						if (curPartidas.commitBuffer()) {
					...
					if ( contador == totalPrepartidas - 1 ) {
						...
						curPartidas.setModeAccess(curPartidas.Edit);
						curPartidas.refreshBuffer();
						curPartidas.setValueBuffer("haber", parseFloat(valorFinal));
						if (this.iface.esDivisaExt == true) {
							if (parseFloat(curPartidas.valueBuffer("tasaconv")) != 0) {
								curPartidas.setValueBuffer("haberme",(parseFloat(valorFinal) / parseFloat(curPartidas.valueBuffer("tasaconv"))));
							}
						}
						if (curPartidas.commitBuffer()) {
							commitOk = true;
			...
			if ( tipoImporte == "Calcular" ) {
				...
				if ( contador == totalPrepartidas - 1 ) {
					...
					curPartidas.setValueBuffer("baseimponible", parseFloat(valorFinal));
					if (curPartidas.commitBuffer()) {
						commitOk = true;
					}
				}
		...
		if ( idACuadrar >= 0) {
			if ( idACuadrar == totalPrepartidas - 1 ) {
				...
				if (parseFloat(descuadreActual) > 0) {
					curPartidas.setValueBuffer("haber", descuadreActual);
					if (cuadraDivisa == true)
						curPartidas.setValueBuffer("haberme", valorFinal);
					if (curPartidas.commitBuffer()) {
						commitOk = true;
					}
				}
				if (parseFloat(descuadreActual) < 0) {
					curPartidas.setValueBuffer("debe",0 -  descuadreActual);
					if (cuadraDivisa == true)
						curPartidas.setValueBuffer("debeme", 0-valorFinal);
					if (curPartidas.commitBuffer()) {
						commitOk = true;
					}
				}
...
function oficial_establecerPaso()
{
	...
	if (datoTipo == "Calcular") {
		valorFinal = parseFloat(this.iface.calculadoraM( valorFinal.toString() ));
		if (esDebe == 1) {
	...
	if (datoTipo == "Último") {
		...
		this.child("fdbTipoDocumento").setValue(flcontppal.iface.pub_getPreMemoria("tipodocumentoanteriorx"));
