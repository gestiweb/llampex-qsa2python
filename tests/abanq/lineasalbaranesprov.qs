...
{	
	...
	query.setSelect("cantidad, totalenalbaran");
	query.setFrom("lineaspedidosprov");
	query.setWhere("idpedido = " + idPedido);
	if (!query.exec())
		return false;

	var estado:String = "Sí";
	var totalServida:Number = 0;
	while (query.next()) {
		var cantidad:Number = parseFloat(query.value(0));
		var cantidadServida:Number = parseFloat(query.value(1));
		totalServida += cantidadServida;
		if (estado == "Sí")
...
{
	var estado:String = this.iface.obtenerEstadoPedido(idPedido);
	if (!estado)
		return false;
...
function oficial_actualizarLineaPedido(idLineaPedido:Number, idPedido:Number, referencia:String, idAlbaran:Number, cantidadLineaAlbaran:Number):Boolean
{
	...
	if (!query.exec())
		return false;
	if (query.next())
		cantidadServida = parseFloat(query.value(0)) + parseFloat(cantidadLineaAlbaran);
