		...
		case cursor.Insert:
			if (last) {
				if (curPagosDevol.valueBuffer("tipo") == "Pago") {
					...
					this.child("fdbCodCuenta").setValue(util.sqlSelect("pagosdevolcli", "codcuenta", "idrecibo = " + cursor.valueBuffer ("idrecibo") + " AND tipo = 'Pago' ORDER BY fecha DESC, idpagodevol DESC"));
					if (this.iface.contabActivada) {
						this.child("fdbCodSubcuenta").setValue(util.sqlSelect("pagosdevolcli", "codsubcuenta", "idrecibo = " + cursor.valueBuffer ("idrecibo") + " AND tipo = 'Pago' ORDER BY fecha DESC, idpagodevol DESC"));
...
function oficial_insertarFacturaDevolProv()
{
	...
	var codBanco:String = util.sqlSelect("cuentasbanco", "entidad", "codcuenta = '" + cursor.valueBuffer("codcuenta") + "'");
	if (!codBanco) {
