...
{ 
	...
	if (cursor.modeAccess() == cursor.Insert) {
		...
		this.child("fdbCodSubcuentaDeu").setValue(util.sqlSelect("co_cuentas c INNER JOIN co_subcuentas s " + 
				...
				" idcuentaesp = 'IVADEU' AND c.codejercicio = '" + this.iface.codEjercicioActual + "' ORDER BY s.codsubcuenta",
				"co_cuentas,co_subcuentas"));
		this.child("fdbCodSubcuentaAcr").setValue(util.sqlSelect("co_cuentas c INNER JOIN co_subcuentas s " + 
				" ON c.idcuenta = s.idcuenta", "s.codsubcuenta", 
				" idcuentaesp = 'IVAACR' AND c.codejercicio = '" + this.iface.codEjercicioActual + "' ORDER BY s.codsubcuenta",
				"co_cuentas,co_subcuentas"));
	}
...
function interna_calculateField(fN:String):String
{
	...
	switch(fN) {
		...
		case "fechainicio": {
			var fechaEjercicio:Date = new Date(Date.parse(this.child("fdbIniEjercicio").value()));
			var fechaInicio:Date = new Date(Date.parse(this.child("fdbFechaInicio").value()));
...
\end */
	...
	var miWhere:String  = "(c.idcuentaesp = 'IVASOP' OR  c.idcuentaesp = 'IVAREP')" + 
