...
\end */
		if (this.iface.mostrarEjAnt) {
			q.setWhere
				("(cbl.naturaleza = 'ACTIVO' OR cbl.naturaleza = 'PASIVO') AND ( buf.sumaact != 0 OR buf.sumaant != 0 ) AND (c.codejercicio = '" + this.iface.ejAct + "' OR c.codejercicio = '" + this.iface.ejAnt + "') GROUP BY buf.sumaact, buf.sumaant, empresa.nombre, cbl.naturaleza, cbl.descripcion1, cbl.descripcion2, cbl.descripcion3, cbl.nivel1, cbl.nivel2, c.descripcion, c.codcuenta");
		} else {
			q.setWhere
				("(cbl.naturaleza = 'ACTIVO' OR cbl.naturaleza = 'PASIVO') AND buf.sumaact != 0 AND c.codejercicio = '" + this.iface.ejAct + "' GROUP BY buf.sumaact, buf.sumaant, empresa.nombre, cbl.naturaleza, cbl.descripcion1, cbl.descripcion2, cbl.descripcion3, cbl.nivel1, cbl.nivel2, c.descripcion, c.codcuenta");
		}

		q.setOrderBy
				("cbl.naturaleza, cbl.descripcion1, cbl.descripcion2, cbl.descripcion3, c.codcuenta");
