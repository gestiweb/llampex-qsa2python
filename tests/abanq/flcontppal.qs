...
class oficial extends interna {
	...
	function comprobarAsiento(idAsiento:String):Boolean {
		return this.ctx.oficial_comprobarAsiento(idAsiento);
	}
}
//// OFICIAL /////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////
...
class ifaceCtx extends head {
	...
	function pub_comprobarAsiento(idAsiento:String):Boolean {
		return this.comprobarAsiento(idAsiento);
	}
}
...
function interna_beforeCommit_co_subcuentas(curSubcuenta):Boolean
{
		...
		case curSubcuenta.Edit:
				if (!curSubcuenta.valueBuffer("codsubcuenta").startsWith(curSubcuenta.valueBuffer("codcuenta"))) {
						MessageBox.warning(util.translate("scripts",
		...
		case curAsiento.Insert: {
			...
			if (curAsiento.modeAccess() == curAsiento.Insert || curAsiento.modeAccess() == curAsiento.Edit) {
				if (util.sqlSelect("co_asientos", "numero", "codejercicio = '" + curAsiento.valueBuffer("codejercicio") + "' AND numero = '" + curAsiento.valueBuffer("numero") + "' AND idasiento <> " + curAsiento.valueBuffer("idasiento"))) {
					...
					var numAsiento:Number = util.sqlSelect("co_asientos", "MAX(numero)",  "codejercicio = '" + curAsiento.valueBuffer("codejercicio") + "'");
					curAsiento.setValueBuffer("numero", ++numAsiento);
				}
			}
		}
	}
...
function oficial_generarPGC(codEjercicio)
{
		...
		var datos =	[["1","1. FINANCIACION BASICA","EPI","1","","1","",""],
				...
				["264","4751. Hacienda Pública, acreedor por retenciones practicadas","CUE","4751","38","47","IRPF","P-E-V-1"],
...
function oficial_generarCodigosBalance()
{
		...
		var datos =
				...
				["D-III-0","D","DEBE","III","AIII. BENEFICIOS DE LAS ACTIVIDADES ORDINARIAS ( AI+AII-BI-BII )","0","A10. Variación de las provisiones de inmovilizado inmaterial, material y cartera de control","",""],
				["D-III-1","D","DEBE","III","AIII. BENEFICIOS DE LAS ACTIVIDADES ORDINARIAS ( AI+AII-BI-BII )","1","A11. Pérdidas procedentes de inmovilizado inmaterial, material y cartera de control","",""],
				["D-III-2","D","DEBE","III","AIII. BENEFICIOS DE LAS ACTIVIDADES ORDINARIAS ( AI+AII-BI-BII )","2","A12. Pérdidas por operaciones con acciones y obligaciones propias","",""],
				["D-III-3","D","DEBE","III","AIII. BENEFICIOS DE LAS ACTIVIDADES ORDINARIAS ( AI+AII-BI-BII )","3","A13. Gastos extraordinarios","",""],
				["D-III-4","D","DEBE","III","AIII. BENEFICIOS DE LAS ACTIVIDADES ORDINARIAS ( AI+AII-BI-BII )","4","A14. Gastos y pérdidas de otros ejercicios","",""],
				...
				["D-V-5","D","DEBE","V","AV. BENEFICIOS ANTES DE IMPUESTOS ( AIII + AIV - BIII - BIV )","5","A15. Impuesto sobre sociedades","",""],
				["D-V-6","D","DEBE","V","AV. BENEFICIOS ANTES DE IMPUESTOS ( AIII + AIV - BIII - BIV )","6","A16. Otros impuestos","",""],
				...
				["H-III-A","H","HABER","III","BIII. PERDIDAS DE LAS ACTIVIDADES ORDINARIAS ( BI + BII - AI - AII )","A","B9. Beneficios en enajenación de inmovilizado inmaterial, material y cartera de control","",""],
				["H-III-B","H","HABER","III","BIII. PERDIDAS DE LAS ACTIVIDADES ORDINARIAS ( BI + BII - AI - AII )","B","B10. Beneficios por operaciones con acciones y obligaciones propias","",""],
				["H-III-C","H","HABER","III","BIII. PERDIDAS DE LAS ACTIVIDADES ORDINARIAS ( BI + BII - AI - AII )","C","B11. Subvenciones de capital transferidas al resultado del ejercicio","",""],
				["H-III-D","H","HABER","III","BIII. PERDIDAS DE LAS ACTIVIDADES ORDINARIAS ( BI + BII - AI - AII )","D","B12. Ingresos extraordinarios","",""],
				["H-III-E","H","HABER","III","BIII. PERDIDAS DE LAS ACTIVIDADES ORDINARIAS ( BI + BII - AI - AII )","E","B13. Ingresos y beneficios de otros ejercicios","",""],
...
function oficial_generarSubcuentas(codEjercicio, longSubcuenta)
{
		...
		while (curCuenta.next()) {
				...
				with(curSubcuenta) {
						...
						setValueBuffer("coddivisa", util.sqlSelect("empresa",
								"coddivisa", "1 = 1"));
						commitBuffer();
				}
...
function oficial_comprobarRegIVA(idSubcuenta, idAsiento):Boolean
{
		...
		var idCuentaEsp:String = util.sqlSelect("co_subcuentas s INNER JOIN co_cuentas c" + 
				" ON s.idcuenta = c.idcuenta", 
				"idcuentaesp",
				"s.idsubcuenta = " + idSubcuenta, 
				"co_subcuentas,co_cuentas");
		if (idCuentaEsp == "IVAREP" || idCuentaEsp == "IVASOP") {
				...
				if (util.sqlSelect("co_regiva", "idregiva",  
						"fechainicio <= '" + fechaAsiento + "'" + 
						" AND fechafin >= '" + fechaAsiento + "' and codejercicio = '" + ejercicioAsiento + "'")) {
						MessageBox.warning(util.translate("scripts", 
								"No puede modificarse una partida de I.V.A. en un período ya regularizado"),
								MessageBox.Ok, MessageBox.NoButton, MessageBox.NoButton);
						return false;
				}
		}
...
\end */
function oficial_comprobarAsiento(idAsiento:String):Boolean
{
	var util:FLUtil = new FLUtil;
	var totalDebe:Number = parseFloat(util.sqlSelect("co_partidas", "SUM(debe)", "idasiento = " + idAsiento));
	if (!totalDebe || isNaN(totalDebe))
		totalDebe = 0;
	var totalHaber:Number = parseFloat(util.sqlSelect("co_partidas", "SUM(haber)", "idasiento = " + idAsiento));
	if (!totalHaber || isNaN(totalHaber))
		totalHaber = 0;
