...
function oficial_lanzar()
{
		...
		var masWhere:String = " AND co_cuentas.idcuentaesp = 'IVAREP' AND (series.siniva IS NULL OR series.siniva = false)";
...
function oficial_recargoEq(nodo:FLDomNode, campo:String):String
{
	...
	var valor:Number = util.sqlSelect("co_partidas p INNER JOIN co_subcuentas s ON p.idsubcuenta = s.idsubcuenta INNER JOIN co_cuentas c ON s.idcuenta = c.idcuenta", "p.haber - p.debe", "idasiento = " + idAsiento + " AND c.idcuentaesp = 'IVAACR' AND p.iva IN (" + porIva + ", 0) AND p.recargo IN (" + porRe + ", 0)", "co_partidas,co_subcuentas,co_cuentas");
	if (!valor)
		valor = 0;
